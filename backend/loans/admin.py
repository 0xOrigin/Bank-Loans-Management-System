...
class LoanPlanAdmin(BankAdmin):
    list_display = BankAdmin.list_display
    search_fields = ['annual_interest_rate', 'duration_in_months']


@admin.register(Loan)
class LoanAdmin(BankAdmin):
    list_display = BankAdmin.list_display + ['purpose', 'amount', 'plan', 'status', 'is_active', 'is_amortized']
...
class LoanPaymentAdmin(BankAdmin):
    list_display = BankAdmin.list_display
